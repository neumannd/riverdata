...
interpolate.river.mom.R = function(inData, from='none', to='none', method = 'step') {
  ...
  if (from == to) {
    ...
    ## testing:
    # inData = list(vals = array(c(2,3,4,4,2,1,1,5,6,5), dim = c(nTime,1)),
    #               time = as.POSIXct(c('1995-10-16 00:00 GMT', '1995-11-16 00:00 GMT',
    ...
    if (from == 'monthly' && to == 'daily') {
      ...
      midPoint = array(0.0, dim = c(nTime,nVars-1))
      crossPoint = array(0.0, dim = c(nTime+1,nVars-1))
      tmpdata = inData$data[,-1]
      midPoint = rbind(tmpdata[1,]*1.1             - tmpdata[2,]*0.1,
                       tmpdata[c(-1,-nTime),]*1.25 - tmpdata[c(-1,-2),]*0.125 - tmpdata[c(-nTime+1,-nTime),]*0.125,
                       tmpdata[nTime,]*1.1         - tmpdata[nTime-1,]*0.1)
      crossPoint = rbind(midPoint[1,],
                         (tmpdata[-nTime,] + tmpdata[-1,])*0.5,
                         midPoint[nTime,])
      
      # TODO: resolve problem with time column!!! 1 and 2 and -1 and nvars-1 indices
      
      startDate = as.character(inData$time[1])
      substr(startDate,9,10) <- '01'
      endDate = as.character(inData$time[nTime])
      ...
      for (iT in 1:nTime) {
        cDate = as.character(inData$time[iT])
        # print(cDate)
        ...
        for (iV in 1:nVars) {
          ...
          tmp_tmp_valsD[,iV] = tmp_tmp_valsD[,iV] * inData$vals[iT,iV] / mean(tmp_tmp_valsD[,iV])
        }
      ...
      # insert data
      otData$data = cbind(tmp_timeD, tmp_valsD)
      
    } else {
      stop("riverInterpolate: only 'from==monthly && to==daily' is implemented")
    }
